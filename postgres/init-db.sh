...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    ...
    GRANT ALL PRIVILEGES ON DATABASE sermng TO "$POSTGRES_USER";
EOSQL

psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    CREATE DATABASE updpay;
    GRANT ALL PRIVILEGES ON DATABASE updpay TO "$POSTGRES_USER";
EOSQL

psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    CREATE DATABASE paymen;
    GRANT ALL PRIVILEGES ON DATABASE paymen TO "$POSTGRES_USER";
EOSQL

psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    CREATE DATABASE suppor;
    GRANT ALL PRIVILEGES ON DATABASE suppor TO "$POSTGRES_USER";
EOSQL
